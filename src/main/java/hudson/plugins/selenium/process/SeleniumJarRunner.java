...
public abstract class SeleniumJarRunner implements SeleniumProcess {
	...
	public void start(Computer computer, TaskListener listener, String name)
			throws IOException, InterruptedException, ExecutionException {
		PluginImpl p = PluginImpl.getPlugin();

		final FilePath seleniumJar = new FilePath(
				SeleniumProcessUtils.findStandAloneServerJar());
	 ...
	 * 
	 * @see
	 * hudson.plugins.selenium.configuration.SeleniumRunner#stop(hudson.model
	 * .Computer)
	 */
	public void stop(Computer computer, String name) {
		...
		if (mainChannel != null) {
			try {
				...
				e.printStackTrace();
				try {
					mainChannel.call(new RunningRemoteSetterCallable(name,
							"Error"));
				} catch (Exception e1) {
					e1.printStackTrace();
				}
